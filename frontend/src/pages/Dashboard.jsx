...
export default function Dashboard() {
  ...
  const { student, placement, scores, breakdowns, modules, analysis, stats, recent_activities, leaderboard } = data
  ...
  // Calculate dynamic gamified streak and target metrics
  const dynamicStreak = Math.max(1, stats.technical_tests_taken + stats.aptitude_tests_taken + (stats.resume_uploaded ? 1 : 0))
  const completedModules = modules.completed || 0
  const totalModules = modules.total || 5
  ...
  // Generate dynamic study planner checklist based on computed weak areas
  const generateStudySchedule = () => {
    const list = []
    const weakNames = (analysis?.weak_areas || []).map(w => w.area)
    
    if (weakNames.includes("DSA & Coding")) {
      list.push({ day: 'MON', time: '10:00 AM', task: 'DSA Algorithms Challenge' })
    } else {
      list.push({ day: 'MON', time: '10:00 AM', task: 'Advanced Coding Practice' })
    }
    
    if (weakNames.includes("Aptitude") || weakNames.includes("Quantitative") || weakNames.includes("Logical Reasoning")) {
      list.push({ day: 'WED', time: '02:00 PM', task: 'Logical & Aptitude Practice' })
    } else {
      list.push({ day: 'WED', time: '02:00 PM', task: 'Complex Puzzle Solving' })
    }
    
    if (weakNames.includes("Interview") || weakNames.includes("Technical Skills")) {
      list.push({ day: 'FRI', time: '04:00 PM', task: 'AI Technical Mock Interview' })
    } else {
      list.push({ day: 'FRI', time: '04:00 PM', task: 'HR Placement Mock Drill' })
    }
    return list
  }
          ...
          <div className="hero-info-section">
            ...
            <div className="hero-gamify-row" style={{ display: 'flex', gap: 24, marginTop: 20 }}>
              <div className="streak-badge-premium">
                ...
                <div>
                  <strong>{dynamicStreak} {dynamicStreak === 1 ? 'Day' : 'Days'}</strong>
              ...
              <div className="weekly-target-badge">
                ...
                <div style={{ flexGrow: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontWeight: 700 }}>
                    ...
                    <span>{completedModules}/{totalModules} Done</span>
                  </div>
                  <div className="score-bar" style={{ marginTop: 4, height: 6 }}>
                    <div className="score-bar-fill" style={{ width: `${completionPercentage}%`, background: 'linear-gradient(90deg, var(--accent-blue), var(--accent-cyan))' }} />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="roadmap-stepper-premium" style={{ marginTop: 20 }}>
            ...
            <div className={`step-item-premium ${probability >= 70 ? 'completed' : 'pending'}`}>
              ...
              <div className="step-content-meta">
                ...
                <p>{probability >= 70 ? 'AI predictions calibrated' : 'Run predictive algorithms'}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card widget-card-premium">
          ...
          <div className="leaderboard-list" style={{ marginTop: 12 }}>
            {leaderboard.length === 0 ? (
              <p className="no-data-text" style={{ padding: '12px 0', color: 'var(--text-secondary)', fontSize: 12, textAlign: 'center' }}>
                No users have completed this assessment yet. Be the first to complete the test and claim Rank #1.
              </p>
            ) : (
              leaderboard.map(u => (
